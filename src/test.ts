...
`, "testcode.txt")
err.setTrace("This is a test error.", new SourceLine({ line: 2, from:  1, length: 1, source: source }))
...
err4.setTrace("an* oops", new SourceLine({ line: 8, from: 9, length: 1, source}))
err5.setTrace(
...
err7.setTrace("It is impossible to capture a newline character", new SourceLine({ line: almost_empty.getLineNumberAt(2), from: 1, length: 1, source: almost_empty}))
